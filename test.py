...
def post_exerciseplan():
	...
	if request.json:
		...
		if temp_name in tables:
			...
			conn = sqlite3.connect('test.sqlite')
			c = conn.cursor()
			sql_query = "CREATE TABLE '{}' (id INTEGER PRIMARY KEY, sex TEXT NOT NULL, weight_kg REAL NOT NULL, height_cm REAL NOT NULL, age INTEGER NOT NULL, exercise TEXT);".format(data)
			c.execute(sql_query)
			conn.commit()
...
def post_exercise(name):
    ...
    if temp_name in tables:
    	if not request.json or not request.json.keys() == {'sex','weight_kg','height_cm','age','exercise'}:
    		...
    		conn = sqlite3.connect('test.sqlite')
    		c = conn.cursor()
    		sql_query = "INSERT INTO '{}' VALUES (null, '{}', '{}', '{}', '{}', '{}');".format(name, data["sex"], data["weight_kg"], data["height_cm"], data["age"], data["exercise"])
    		c.execute(sql_query)
    		conn.commit()
